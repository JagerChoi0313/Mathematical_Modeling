...
% 图2适合正文或附录；图5~图7适合模型解释、检验和稳健性分析。
if opts.makeFigures
...
for k=1:n
    ...
    plot(ax,find(optimal(k,:)),profit(k,optimal(k,:)),'o','LineStyle','none', ...
        'MarkerEdgeColor',[0.04,0.30,0.17],'MarkerFaceColor','none', ...
    ...
    for j=find(optimal(k,:))
        text(ax,j,profit(k,j)+0.035*span,sprintf('%.2f',profit(k,j)), ...
            'HorizontalAlignment','center','FontName',font,'FontSize',8.5, ...
            'Color',[0.04,0.30,0.17]);
    end
...
ax1=nexttile(layout); b1=bar(ax1,x,E(:,1:4),0.76,'grouped','EdgeColor','none');
colors1=[0.23,0.43,0.63;0.45,0.65,0.78;0.30,0.55,0.73;0.58,0.76,0.84];
for k=1:4, b1(k).FaceColor=colors1(k,:); end
setScenarioX(ax1,R); ylabel(ax1,'期望次数（次/订单）','FontName',font);
title(ax1,'零配件采购与检测','FontName',font,'FontSize',12);
legend(ax1,{'采购零件1','采购零件2','检测零件1','检测零件2'}, ...
    'FontName',font,'Location','northoutside','Orientation','horizontal');
styleAxis(ax1,font);

ax2=nexttile(layout); b2=bar(ax2,x,E(:,5:8),0.76,'grouped','EdgeColor','none');
...
b=bar(ax,1:n,top,0.70,'grouped','EdgeColor','none'); hold(ax,'on');
...
xlabel(ax,'差值越小，说明最优方案对参数变化可能越敏感','FontName',font);
title(ax,{'最高利润与第二高利润策略比较', ...
    '第二名允许与第一名并列，因此情形3的差值为0'}, ...
    'FontName',font,'FontSize',15,'Interpreter','none');
...
h2=plot(ax1,x,T.ExactCost,'d-','Color',[0.74,0.32,0.22], ...
...
setScenarioX(ax2,R); ylim(ax2,[-2.6,2.6]);
